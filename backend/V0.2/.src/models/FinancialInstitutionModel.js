...
class FinancialInstitutionModel extends BaseModel {
    ...
    async initializeData() {
        ...
        try {
            ...
            if (isEmpty) {
                ...
                logger.info('Financial institutions table is not empty. Skipping CSV import.');
            }
        } catch (error) {
    ...
    async _isTableEmpty() {
        ...
        const result = await pgClient.query(`SELECT COUNT(*) FROM ${this.tableName}`);
    ...
    async _importCSVData() {
        ...
        try {
            ...
            for (const data of results) {
                ...
                if (String(data.fi_code)?.trim() && String(data.name_th)?.trim() && String(data.name_en)?.trim()) {
                ...
                logger.silly(`Attempting to insert CSV data to the Database: ${sanitizedData.fi_code}`);
                ...
                try {
                    ...
                    await pgClient.query(sql, [sanitizedData.fi_code, sanitizedData.name_th, sanitizedData.name_en]);
                    logger.silly(`CSV data added: ${JSON.stringify(sanitizedData)}`);
                } catch (error) {
                    if (error.code === '23505') {
                        logger.warn(`Duplicate entry for fi_code: ${sanitizedData.fi_code}, skipping this row.`);
                    } else {
