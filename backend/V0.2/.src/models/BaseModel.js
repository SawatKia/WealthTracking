...
class BaseModel {
    ...
    async update(primaryKeys, data) {
        return this.executeWithTransaction(async () => {
            ...
            const conditionPlaceholders = keys.map((_, index) => `$${updateKeys.length + index + 1}`).join(' AND ');

            const sql = `UPDATE ${this.tableName} SET ${updatePlaceholders} WHERE ${keys.join(' = ')} = ${conditionPlaceholders} RETURNING *`;
            const result = await this.pgClient.query(sql, [...updateValues, ...primaryValues]);
    ...
    async delete(primaryKeys) {
        return this.executeWithTransaction(async () => {
            ...
            const sql = `DELETE FROM ${this.tableName} WHERE ${keys.join(' = ')} = ${placeholders} RETURNING *`;
            const result = await this.pgClient.query(sql, values);
